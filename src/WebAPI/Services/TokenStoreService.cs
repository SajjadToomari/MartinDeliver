...
public class TokenStoreService : ITokenStoreService
{
    ...
    public Task AddUserTokenAsync(User user, string refreshTokenSerial, string accessToken,
    ...
    {
        var now = DateTimeOffset.UtcNow;
        return _tokens.Where(x => x.RefreshTokenExpiresDateTime < now)
    ...
    public async Task RevokeUserBearerTokensAsync(string? userIdValue, string refreshTokenValue)
    {
        if (!string.IsNullOrWhiteSpace(userIdValue) &&
            int.TryParse(userIdValue, NumberStyles.Number, CultureInfo.InvariantCulture, out var userId) && _configuration.Value.AllowSignoutAllUserActiveClients)
        {
    ...
    {
        return _tokens.Where(x => x.UserId == userId)
                     .ForEachAsync(userToken => _tokens.Remove(userToken));
    }
